...
import com.vincent.projectanalysis.download.Task.TaskListener;
import com.vincent.projectanalysis.download.task.TaskFactory;
...
public class DownloadManager {
    ...
    public Task getTaskById(String taskId) {
        ...
        for (int i = 0; i < finishedList.size(); i++) {
            Task task = finishedList.get(i);
            if (taskId.equals(task.getTaskId())) {
                File file = new File(task.getDestFilePath());
                if (!file.exists()) {
                    DataBaseManager.getDataBaseManager().getTable(DownloadTable.class)
                            .removeDownload(task.getTaskId());
                    return null;
                }
